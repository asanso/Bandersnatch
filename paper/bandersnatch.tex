...
The GLV method~\ref{GLV} is a well known trick for accelerating the
...
In this work, we are looking for ordinary elliptic curves~\ref{Silv?}
...
of degree $O(D)$ thanks to the Vélu's formulas~\ref{velu}.
...
attacks~\ref{subgroup-attack, twist-subgroup-attack}, meaning that the
order of the curve and its quadratic twist has a large prime factor.
...
($-388 \leq -D \leq -3$) in \href{???}{this file}, and we finally
obtain an interesting curve for $-D = -2 \cdot 4$.
