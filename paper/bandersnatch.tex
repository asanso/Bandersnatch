...
\titlerunning{Bandersnatch: a fast elliptic curve built over the BLS12-381
scalar field}
\author{ Simon Masson  \and Antonio Sanso   \and Zhenfei Zhang 
}
\institute{Anoma \and 
Ethereum Foundation and Ruhr Universität Bochum \and Ethereum Foundation }
...
\noindent{\em Keywords: Elliptic curve cryptography}\\
\noindent{\em MSC Code: 94A60}
...
We compute an exhaustive search following the CM method [CITE PAPER].
...
\begin{algorithmic}[1]
    ...
    \ForEach {$D \in \{D_{\min}, \hdots, D_{\max}\}$}
        ...
        \ForEach{$j_0\in R$}
            \ForEach{$E$ elliptic curve of $j$-invariant $j_0$ (up to isomorphism)}
                \If{$E$ is not supersingular}
...
We list in Table 2 the ordinary curves we obtained. In this table,
pn denotes a prime of n bits. The generation of these curves is reproducible
...
\begin{algorithm}
  ...
  \State $(x_0,...x_{254})\gets \texttt{BitDecompose(x)}$ \Comment{Decomposition circuit}
  \State $H' = \texttt{InfinityPoint}$
  \For {$i\in [0,254]$} 
    \If{$x_{254-i}==1$}         \Comment{Binary Selection circuit}
      \State  $H' \gets H' + G$   \Comment{Affine Curve Group Add circuit}
    \EndIf
    \State $H' \gets H' + H'$      \Comment{Affine Curve Group Double circuit}
  \EndFor
  \State $\texttt{EnforcePointEq}(H, H')$ \Comment{Equality circuit}
  \end{algorithmic}
\end{algorithm}
