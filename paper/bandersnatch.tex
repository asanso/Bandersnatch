...
\begin{abstract}
 ...
 compared to another curve called Jubjub, having similar properties. Nonetheless,
 Bandersnatch to not provide any performance improvement for either 
 rank 1 constraint systems (R1CS) or multi-scalar-multiplications, compared to
 the Jubjub curve.
...
the cost of group multiplication over the default generators.
...
\ZZ{I incline to remove this subsection completely}
We also implemented scalar multiplications with fixed bases. 
In short, for fixed base scalar multiplications, having an 
endomorphism does not
improve the overall performance. Nonetheless, it is worth
pointing out that the endomorphism does improve the storage
size by a factor of 2, for a fixed window size.
...
consume another 10 constraints). 
...
\section{Conclusion}
Last decade has seen great improvement on practical zk-snark systems.
One of an essential stepping stone is an efficient elliptic curve whose base field
matches the scalar  field for some pairing friendly curve. 
In this note, we present Bandersnatch as an alternative to the common used
base curve, Jubjub. Due to the existence of GLV endomorphism, group 
operation over our curve is 42\% times faster than that of Jubjub curve.
For multi-scalar-multiplications, we report a narrowed advantage over Jubjub 
curve when the dimension is small; and no advantage for medium to large dimensions.
