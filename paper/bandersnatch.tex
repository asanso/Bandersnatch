...
We also implemented scalar multiplications with fixed bases, where
window techniques reduce the overall cost using a precomputation
table.
A scalar multiplication with a window $w$ requires the
precomputation of $2^w$ points, and then reduces the doubling steps by
a factor $w$.
Using the GLV method, two scalars are read in a row, and so using a
window $w$, the precomputation table has $2^{2w}$ elements.
In the context of a fixed precomputation table size, the window in the
GLV method need to be halved and the gain of the GLV method does not
compensate the factor $w$ improvement in the basic scalar multiplication.
In short, Jubjub is as fast as Bandersnatch in the case of a fixed
base point.
