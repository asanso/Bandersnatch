...
Multi-scalar-multiplications. Note that this MSM is 
compatible, but
different
from the MSM inside the GLV. 
In particular, for a $k$ scalar multiplications,
...
\begin{itemize}
  ...
  \item use GLV endomorphism to break the $k$ base-scalar pairs into 
  $2k$ number of new base-scalar pairs, where the scalar are half sized,
  i.e., 128 bits.
\end{itemize}
The data are showing in Figure \ref{fig:msm}. Specifically, 
as a baseline, the trivial solution, captained by 
{\em GLV without 
MSM}, is a simple extrapolation of number of bases times the cost of
...
than 4, MSM algorithms begins to out-perform trivial solutions.
...
We want to evaluate the number of constraints for a 
variable base group multiplication. For a double-then-add
algorithm, it takes 6 constraints per addition, 5 constraints
